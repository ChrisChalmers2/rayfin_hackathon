...
## Coding Conventions (Jobsite Twin)

These conventions reflect patterns established while building this app. Follow them when extending it.

- **Sentinel substitution for parameterized DAX.** A `.dax` file holds the base query with a `"__PROJECT_ID__"` sentinel; the factory `.ts` replaces it via a shared `applyProjectId` helper that **doubles embedded double-quotes** to keep the DAX string literal intact (a light injection guard). Never string-concatenate raw user input into DAX; route it through the sentinel helper. Structural query changes get separate `.dax` variant files, not inline query strings.
- **Pure-function extraction + colocated specs.** Any non-trivial logic that can be pure (row mapping, sort-key parsing, geometry math) is extracted into a plain `.ts` module and unit-tested. Examples: `mapZones`, `computeZoneStackOrder`, `columnHeight`/`columnPositions`. Rule of thumb: **if you add a new pure function, add a `.spec.ts` for it**. Trivial one-line inline maps do not require extraction.
- **Spec file convention.** Tests live next to the code as `<name>.spec.ts(x)`, run under Vitest. Query factories have a colocated spec asserting the query contains the expected DAX and the `columnMetadata` maps the exact CLI-observed column names. Keep the full suite green; add cases rather than loosening assertions.
- **Shared lookups via memoized maps.** When two views need the same derived data, shape it once with a pure helper and derive lookup maps with `useMemo` off already-loaded state — never issue a second fetch. Example: the zones fetch feeds both `zonesById` (id → Zone) and `locationIdByZoneName` (zone name → id), the latter enabling the tasks-grid row-click → 3D selection.
- **Single source of truth for selection.** Cross-component selection state (e.g. `selectedLocationId`) lives at the nearest common parent with a toggle-to-clear setter; children receive the value + a callback. Re-clicking the selected item clears it.
- **DataGrid interaction gotcha.** `DataGrid` builds `onInteraction` select predicates **only from the columns in its `columns` prop**, not from every field of the datum. To drive selection off a field, it must be a rendered column (that is why the tasks grid keys selection off the visible `ZoneName` column, not a hidden id).
- **Manual file pre-creation via terminal.** When the file-creation tool is unavailable, create new files with a PowerShell single-quoted here-string (`Set-Content -Path <file> -Value @'...'@`). Edit existing files with the editing tools, not the terminal.
- **Diagnose-before-fix workflow.** For bug reports, prove the root cause first (read the actual library/source, cite file+line) and report findings before changing code. Prefer definitive source evidence over speculative fixes.
- **Docs hygiene for public showcase.** No customer names, workspace/dataset IDs, or deployment URLs in committed docs or code comments. Refer to the app generically ("construction general contractor", "Jobsite Twin"). Public technical patterns (local hash routing, r3f, drei, DAX) are fine.
- **Synthetic data lives in the notebook, not the app.** The demo Delta tables are generated by `notebooks/01_synthetic_data_construction.ipynb` (repo root) — deterministic (SEED=42), idempotent. Task→zone placement uses a `TASK_ZONE_AFFINITY` map with a Spark-native, seeded `xxhash64` `row_number()` pick (reproducible, columnar — no UDF) plus a `ZONE_HEIGHT` nearest-by-height fallback; Section 9 assertions gate data quality. Data-layer fixes belong in the notebook — never hardcode data in the app.
